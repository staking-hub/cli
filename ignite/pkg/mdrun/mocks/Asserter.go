...
// Assert provides a mock function with given fields: cmd, codeBlock
func (_m *Asserter) Assert(cmd string, codeBlock mdrun.CodeBlock) error {
	ret := _m.Called(cmd, codeBlock)

	var r0 error
	if rf, ok := ret.Get(0).(func(string, mdrun.CodeBlock) error); ok {
		r0 = rf(cmd, codeBlock)
	} else {
		r0 = ret.Error(0)
	}
...
func (_e *Asserter_Expecter) Assert(cmd interface{}, codeBlock interface{}) *Asserter_Assert_Call {
	return &Asserter_Assert_Call{Call: _e.mock.On("Assert", cmd, codeBlock)}
}

func (_c *Asserter_Assert_Call) Run(run func(cmd string, codeBlock mdrun.CodeBlock)) *Asserter_Assert_Call {
	_c.Call.Run(func(args mock.Arguments) {
		run(args[0].(string), args[1].(mdrun.CodeBlock))
	})
	return _c
}
